...
- **`scripts/registry.py` — the model registry, single source of truth (pure data).** A `MODELS` dict maps each organism name to a config: `provider`, `checkpoint`/`checkpoint_file`/`model_id`, `api_key_env`, `project_id`, `reasoning` mode, `system_prompt_id`, and ground-truth `misalignment`. It is **importable with only the stdlib — no tinker/transformers/requests and no disk/env reads at import** — so `score_batch`/`verify_probe` can resolve ground truth without the heavy sampling machinery. Helpers: `canonical(key)` / `get(key)` / `misalignment_for(key)` apply the `ALIASES` map (renamed keys → current key: `daniel-tan-s2`→`covert-manipulator`, `O2`/`tinker-O2`→`O2-schemer`) so old batches still resolve; `resolve_checkpoint(cfg)` reads a `checkpoint_file` (em-qwen3) lazily at call time, never at import. The `build_*_caller` functions deliberately stay in `run_batch`, not here, so importing the registry stays dependency-light.
- **`scripts/run_batch.py` — the registry-driven batch runner.** Loads `registry.MODELS`; pick an organism with `--model <name>` and it dispatches to the right caller. Every `build_*_caller` returns a `caller(system, user, max_tokens, temperature)` yielding the *same normalized dict* (`response`, `reasoning`, `finish_reason`, `truncated`, `usage`, …), so logging never branches on provider. A generation row records only the model **key** (`model_requested`), **not** the misalignment string — ground truth is resolved from the registry at score time (single source of truth). Three providers:
